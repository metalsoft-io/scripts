...
This demo builds a two scalable-unit Spectrum-X fabric from a clean state and attaches
a tenant to it, using the MetalSoft CLI and Terraform. The fabric is a two-tier
leaf-spine design running EVPN over eBGP on Cumulus Linux 5.14.0. It extends the
single-unit build with cross-unit reachability: a host in one unit reaches a host in
the other over the EVPN overlay. Allow about 45 minutes end to end; most of that is
switch deployment time.
...
| HGX hosts | `192.168.200.23` to `.30` | Ubuntu compute nodes, eight rail NICs each |
...
| Switches | `ssh cumulus@<switch-ip>` | `cumulus` | set in `switches.2su.yaml` |
...
| Property | Value |
|---|---|
| Switches | 12 (8 leaf, 4 spine) |
| HGX hosts | 8 (512 GPUs) |
| Fabric label | `spectrumx-2su-514` |
...
Keep the fabric label `spectrumx-2su-514`. The Terraform manifest in Step 11 resolves
...
Log in as `cumulus` (the password is read from `switches.2su.yaml`) and capture the
...
Set `fabricId` in `fabric-config.2su.l3evpn.yaml` to `1`. This step assigns
...
(`leaf-su00-r0` is `10.253.128.1`, `r1` is `.2`, and so on, continuing into the second
unit), and `nv config show -o commands` returns the full configuration where the baseline
showed only defaults.
...
`fabric-config.2su.l3evpn.yaml` names the Cumulus 5.14 templates under
...
including the cross-unit EVPN routes.
...
place of `Idle` or `Active`. The EVPN overlay runs between the leaves and the relay spines
`spine-s00` and `spine-s01`, which re-advertise the type-5 routes across both units, so
run the EVPN summary against a leaf or one of those two spines to see the overlay sessions.

## Step 9: Register the HGX endpoints

Register the eight HGX hosts as endpoints. Each host binds to eight leaf ports, one
per rail. The four hosts of the first unit sit on `leaf-su00-r*` and the four of the
second unit on `leaf-su01-r*`. The definitions are in `endpoints.2su.yaml`:
...
```bash
metalcloud-cli route-domain create --config-source route-domain.2su.yaml
```

```bash
metalcloud-cli logical-network-profile create vxlan --config-source l3-profile.2su.yaml
```
...
The Terraform manifest `terraform/nvidia-spectrumx-2su.tf` creates the tenant
...
SW=leaf-su00-r0    # a leaf; see the leaf list in Step 3
SWIP=$(awk -v s="$SW" '$2=="identifierString:" && $3==s{f=1} f && $1=="managementAddress:"{print $2; exit}' ~/nvidia/switches.2su.yaml)
SWPW=$(awk '/managementPassword:/{print $2; exit}' ~/nvidia/switches.2su.yaml)
...
the remote ones learned over EVPN, including the rails in the other unit.
...
For host node `h` on rail `r` in unit `u`, the host address is
`172.(16 + 2*r).u.(2*h)` and the gateway is one higher. The rail sets the second octet,
the unit sets the third (SU0 = 0, SU1 = 1), and the node sets the fourth (h00 = 0,
h08 = 16, h16 = 32, h24 = 48):
...
The complete per-host netplan files are staged on the jumpstation under `netplan/2su/`,
one per host (`hgx-su00-h00.yaml` through `hgx-su01-h24.yaml`). Each holds all eight
rails and installs to `/etc/netplan/60-spectrum-x.yaml` on its host.

The hosts are reachable at `192.168.200.23` through `.30` as user `ubuntu` (SU0 hosts
`.23` to `.26`, SU1 hosts `.27` to `.30`).
...
the full rail mesh on each one, covering all eight rails across both units and all four
hosts (64 targets per host, including the same-rail peers in the other unit). Every
...
Reaching a same-rail host in the other unit (for example `172.16.0.0` to `172.16.1.0`)
routes leaf to spine to leaf over the EVPN overlay. A clean fabric prints eight
green-labelled `host mesh: 64 passed, 0 failed` lines and nothing else. BGP and EVPN can
still be converging in the first seconds after the deploy, so the sweep retries each
target for up to about thirty seconds rather than reporting a false failure; anything
still unreachable after that is listed as a red `FAIL <ip>` line under the host that could
not reach it. Re-running the block is safe.
...
- **`get-ports` is empty in Step 3.** The switch is unreachable. Verify the management
  address and password in `switches.2su.yaml`, then re-run discovery.
