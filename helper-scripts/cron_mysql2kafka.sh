...
dbname="$($k exec deploy/mysql -- mysql -BNe 'show databases'|grep -E '^metalsoft$'|head -1)"
test -z "$dbname" && dbname="$($k exec deploy/mysql -- mysql -BNe 'show databases'|grep -E '^bsi_api_'|head -1)"
